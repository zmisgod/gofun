...
func main() {
	...
	page := 1
	for stop == false {
		res, err := dbCon.Query(fmt.Sprintf("select id,train_no,first_telecode,end_telecode from crh_train_group where service_type = '' order by id desc limit %d, %d", (page-1)*pageSize, pageSize))
		checkError(err)
		defer res.Close()
		j := 0
		for res.Next() {
...
func updatelistGroup(trainClassName, serviceType string, groupID int) {
	stmt, err := dbCon.Prepare("update crh_train_group set service_type =? , train_class_name =? where id = ?")
	defer stmt.Close()
	checkError(err)
	_, err = stmt.Exec(serviceType, trainClassName, groupID)
	checkError(err)
}

//crh_train_group_details 新增crh_train_group表
func insertListGroup(detail Detail, groupID int) {
	stmt, err := dbCon.Prepare("insert into crh_train_group_details (group_id,arrive_time,station_name, start_time, stopover_time,station_no, isEnabled) value (?,?,?,?,?,?,?)")
	defer stmt.Close()
	checkError(err)
	isEnable := 0
	if detail.IsEnabled {
