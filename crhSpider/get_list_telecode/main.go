...
func main() {
	...
	for stop == false {
		sql := fmt.Sprintf("select first_station,end_station,id from crh_train_list where (end_telecode = '' or first_telecode = '') limit %d, %d", (i-1)*PAGESIZE, PAGESIZE)
		...
		for res.Next() {
			...
			updateTelecode(trainName.ID, updateSQL, "crh_train_list")
			count++
		}
		if count == 0 {
			stop = true
